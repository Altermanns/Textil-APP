  ...
  return (
    <div className="bg-white shadow-md rounded-lg p-6">
      <h1 className="text-3xl font-bold mb-4">
        Detalles de Hilatura: {hilatura.id}
      </h1>

      <div className="mb-4">
        <p>
          <strong>Preparación de Origen:</strong>{" "}
          {hilatura.preparacion_origen_display || "N/A"}
        </p>
        <p>
          <strong>Etapa:</strong> {hilatura.etapa}
        </p>
        <p>
          <strong>Estado:</strong> {hilatura.estado}
        </p>
        <p>
          <strong>Cantidad de Fibra de Entrada:</strong>{" "}
          {hilatura.cantidad_fibra_entrada} kg
        </p>
        <p>
          <strong>Cantidad de Hilo Producido:</strong>{" "}
          {hilatura.cantidad_hilo_salida} kg
        </p>
        <p>
          <strong>Título del Hilo:</strong> {hilatura.titulo_hilo || "N/A"}
        </p>
        <p>
          <strong>Torsión:</strong>{" "}
          {hilatura.torsion !== null ? `${hilatura.torsion} TPM` : "N/A"}
        </p>
        <p>
          <strong>Resistencia:</strong>{" "}
          {hilatura.resistencia !== null
            ? `${hilatura.resistencia} cN/tex`
            : "N/A"}
        </p>
        <p>
          <strong>Observaciones:</strong>{" "}
          {hilatura.observaciones || "Sin observaciones"}
        </p>
        <p>
          <strong>Calidad del Resultado:</strong>{" "}
          {hilatura.calidad_resultado || "N/A"}
        </p>
        <p>
          <strong>Fecha Inicio:</strong>{" "}
          {new Date(hilatura.fecha_inicio).toLocaleString()}
        </p>
        <p>
          <strong>Fecha Completado:</strong>{" "}
          {hilatura.fecha_completado
            ? new Date(hilatura.fecha_completado).toLocaleString()
            : "Pendiente"}
        </p>
        <p>
      ...
      <div className="flex space-x-4">
        <Link
          href={`/hilaturas/${hilatura.id}/edit`}
